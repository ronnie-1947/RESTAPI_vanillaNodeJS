...
handlers._checks.post = (data, callback)=>{
    ...
    const methhod = typeof(data.payload.methhod)==='string' &&  ['get', 'post', 'put', 'delete'].includes(data.payload.methhod) ?data.payload.methhod.trim():false;
    const successCodes = typeof(data.payload.successCodes)==='object' && data.payload.successCodes instanceof Array &&  data.payload.successCodes.length>0 ?data.payload.successCodes.trim():false;
    const timeoutSeconds = typeof(data.payload.timeoutSeconds)==='number' &&  data.payload.timeoutSeconds%1===0 && data.payload.timeoutSeconds >=1 && data.payload.timeoutSeconds <= 5 ?data.payload.timeoutSeconds.trim():false;

    if(!protocol || !url || !methhod || !successCodes || !timeoutSeconds){
